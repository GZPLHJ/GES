...
class PathHandler:
    ...
    def __init__(self, pred_dir: Path, targ_dir: Path, img_id: str,
        ...
        assert isinstance(
            pred_dir,
            Path), f"pred_dir should be of type Path, got {type(pred_dir)}"
        assert pred_dir.is_dir(
        ), f"Directory '{pred_dir}' does not exist or is not a directory"

        assert isinstance(
            targ_dir, Path
        ), f"targ_dir '{targ_dir}' should be of type Path, got {type(pred_dir)}"
        assert targ_dir.is_dir(
        ), f"Directory '{targ_dir}' does not exist or is not a directory"

        assert test_hold in [
            'test', 'hold'
        ], f"test_hold '{test_hold}' was not one of 'test' or 'hold'"

        self.lp = pred_dir / f"{img_id}_dam_pred.png"  # localization prediction
        self.dp = pred_dir / f"{img_id}_dam_pred.png"  # damage prediction
        self.lt = targ_dir / f"{img_id}_dam_target.png"  # localization target
        self.dt = targ_dir / f"{img_id}_dam_target.png"  # damage target
    ...
    def load_and_validate_image(self, path):
        ...
        assert set(np.unique(img)) <= {
            0, 1, 2, 3, 4
        }, f"values must ints 0-4, found {np.unique(img)}, path: {path}"
...
class RowPairCalculator:
    ...
    @staticmethod
    def compute_tp_fn_fp(pred: np.ndarray, targ: np.ndarray,
                         c: int) -> List[int]:
        """
    ...
    def get_row_pair(cls, ph: PathHandler):
        ...
        lp_b, lt_b, dt_b = map(
            cls.extract_buildings,
            (lp, lt, dt))  # convert all damage scores 1-4 to 1

        dp = dp * lp_b  # only give credit to damages where buildings are predicted
        dp, dt = dp[dt_b == 1], dt[
            dt_b ==
            1]  # only score damage where there exist buildings in target damage
...
class XviewMetrics:
    ...
    def get_path_handlers(self):
        self.path_handlers = []
        test_hold = 'hold'
        for path in self.pred_dir.glob('*.png'):
            #            test_hold, loc_dmg, img_id, target = path.name.rstrip('.png').split('_')
            #            disaster, img_num, loc_dmg, target = path.name.rstrip(
            #                '.png').split('_')
            split_up_pn = path.name.rstrip('.png').split('_')
            if len(split_up_pn) == 5:
                disaster, img_num, build_id, loc_dmg, target = split_up_pn
                img_id = disaster + "_" + img_num + "_" + build_id
            if len(split_up_pn) == 4:
                disaster, img_num, loc_dmg, target = split_up_pn
                img_id = disaster + "_" + img_num

            assert loc_dmg in [
                'dam'
            ], f"target filenames must have 'dam_target' in filename, got {path}"
            assert target == 'pred', f"{target} should equal 'target' when getting path handlers"
            if loc_dmg == 'dam':  # localization or damage is fine here
                self.path_handlers.append(
                    PathHandler(self.pred_dir, self.targ_dir, img_id,
                                test_hold))
    ...
    def get_dfs(self):
        ...
        """
        with Pool() as p:
            all_rows = p.map(RowPairCalculator.get_row_pair,
                             self.path_handlers)

        lcolumns = ['lTP', 'lFN', 'lFP']
        self.ldf = pd.DataFrame([lrow for lrow, drow in all_rows],
                                columns=lcolumns)

        dcolumns = [
            'dTP1', 'dFN1', 'dFP1', 'dTP2', 'dFN2', 'dFP2', 'dTP3', 'dFN3',
            'dFP3', 'dTP4', 'dFN4', 'dFP4'
        ]
        self.ddf = pd.DataFrame([drow for lrow, drow in all_rows],
                                columns=dcolumns)
    ...
    @classmethod
    def compute_score(cls, pred_dir, targ_dir, out_fp=None):
        """
        Args:
